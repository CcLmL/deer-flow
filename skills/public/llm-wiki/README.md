...
1. **LLM Wiki desktop app installed and running.** The API binds to `10.0.27.42:19828` only while the app is open. If the app isn't running, the agent will see `connection refused` and tell you.
...
curl -s http://10.0.27.42:19828/api/v1/health
...
curl -s -H "Authorization: Bearer $LLM_WIKI_API_TOKEN" \
  http://10.0.27.42:19828/api/v1/projects

# 3. Search the active project
curl -s -H "Authorization: Bearer $LLM_WIKI_API_TOKEN" \
  -H 'Content-Type: application/json' \
  -d '{"query":"rope","topK":3}' \
  http://10.0.27.42:19828/api/v1/projects/current/search

# 4. Read a page
curl -s -H "Authorization: Bearer $LLM_WIKI_API_TOKEN" \
  "http://10.0.27.42:19828/api/v1/projects/current/files/content?path=wiki/concepts/rope.md"
```
...
| Symptom | Cause | Fix |
|---|---|---|
| `curl: (7) Failed to connect to 10.0.27.42 port 19828` | App not running | Launch LLM Wiki desktop, wait ~2s for the API server to bind. |
...
- The API listens on `10.0.27.42` only. It is **not reachable from other hosts** on your network.
